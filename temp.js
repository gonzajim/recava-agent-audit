├»┬╗┬┐document.addEventListener('DOMContentLoaded', function() {
  // ========================================================================
  // ### 1) CONFIGURACI├â┬ôN DE FIREBASE ###
  // ========================================================================
  ...
  async function configureEnvironment() {
    ...
    try {
      ...
      if (firebaseConfigFromHosting?.projectId === 'recava-auditor') {
        ...
        console.log('Entorno de Producci├â┬│n detectado.');
      } else {
        currentEndpoints = endpoints.dev;
        console.log('Entorno de Desarrollo detectado.');
      }
    } catch (error) {
      console.warn('No se pudo detectar el entorno desde __firebase/init.json; se usar├â┬ín los endpoints de desarrollo por defecto.', error);
      currentEndpoints = endpoints.dev;
    }
  }
  ...
  // ========================================================================
  // ### 3) HELPERS: verificaci├â┬│n, reset y token verificado ###
  // ========================================================================

  async function sendVerificationIfNeeded(user) {
    try { if (user && !user.emailVerified) { await user.sendEmailVerification(); } }
    catch (e) { console.error("No se pudo enviar el email de verificaci├â┬│n:", e); }
  }
  ...
  // ========================================================================
  // ### 4) BANNER DIN├â┬üMICO PARA VERIFICACI├â┬ôN ###
  // ========================================================================
    ...
    try {
      if (id === 'btn-verify-retry') {
        ...
        if (ok) {
          ...
          if (loginErrorEl) {
            loginErrorEl.textContent = "Tu email sigue sin estar verificado. Revisa el buz├â┬│n o reenv├â┬¡a el correo.";
        ...
        if (loginErrorEl) {
          loginErrorEl.textContent = "Hemos reenviado el email de verificaci├â┬│n.";
          loginErrorEl.style.display = 'block';
        }
        ...
        if (loginErrorEl) {
          loginErrorEl.textContent = "Te hemos enviado un enlace para restablecer tu contrase├â┬▒a.";
          loginErrorEl.style.display = 'block';
        }
      }
    } catch (err) {
      if (loginErrorEl) {
        loginErrorEl.textContent = "Error: " + (err?.message || "Operaci├â┬│n no completada");
        loginErrorEl.style.display = 'block';
      }
    }
  });
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      ...
      currentUser = null;
      console.log("Ning├â┬║n usuario autenticado.");
      if (verifyBanner) verifyBanner.style.display = 'none';
      if (loginViewEl) loginViewEl.style.display = 'block';
      if (chatWrapperEl) chatWrapperEl.style.display = 'none';
      if (chatMessagesEl) chatMessagesEl.style.display = 'none';
  ...
  if (loginButtonEl) {
    loginButtonEl.addEventListener('click', async () => {
      ...
      if (!email || !password) {
        loginErrorEl.textContent = "Por favor, introduce email y contrase├â┬▒a.";
        loginErrorEl.style.display = 'block';
        return;
      }
      try {
  ...
  async function initializeChatInterface() {
    ...
    addAssistantMessageInternal(`├é┬íHola ${currentUser.email}! Somos tus auditores legales especializados.<br>Elige el modo en el que quieres interactuar:`);
  ...
  function handleModeSelectionClick(event) {
    ...
    if (currentChatMode === 'auditor') {
      ...
      addAssistantMessageInternal("Has seleccionado el modo <strong>ASESOR</strong>.<br>├é┬┐En qu├â┬® puedo ayudarte hoy?");
  ...
  async function handleSendMessageToServer() {
    ...
    if (!currentUser) {
      addSystemMessageToChat("Error de autenticaci├â┬│n. Por favor, recarga la p├â┬ígina.");
      return;
    }
    ...
    try {
      ...
      removeTypingIndicatorFromChat();
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ error: "Error de red o respuesta no v├â┬ílida.", details: `Status ${response.status}` }));
      ...
      if (dataFromBackend.response) {
        const cleanedResponseText = dataFromBackend.response.replace(/├â┬ú├ó┬é┬¼├é┬É.*?├â┬ó├ó┬é┬¼├é┬ásource├â┬ú├ó┬é┬¼├ó┬Ç┬ÿ/g, '').trim();
  ...
  if (attachFileButtonEl) { attachFileButtonEl.addEventListener('click', () => addSystemMessageToChat("La funcionalidad de adjuntar archivos se gestiona autom├â┬íticamente por el asistente."));}
  ...
  // No llamamos a initializeChatInterface() aqu├â┬¡; lo hace onAuthStateChanged.
});
